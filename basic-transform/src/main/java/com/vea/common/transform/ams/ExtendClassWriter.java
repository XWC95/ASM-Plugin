 ...
 * @since 2019-01
 */
public class ExtendClassWriter extends org.objectweb.asm.ClassWriter {

    private ClassLoader urlClassLoader;

    private static final String OBJECT = "java/lang/Object";

    public ExtendClassWriter(ClassLoader classReader, int i) {
        super(i);
    ...
    protected String getCommonSuperClass(final String type1, final String type2) {
        ...
        org.objectweb.asm.ClassReader type1ClassReader = getClassReader(type1);
        org.objectweb.asm.ClassReader type2ClassReader = getClassReader(type2);
    ...
    private boolean isImplements(final String interfaceName, final org.objectweb.asm.ClassReader classReader) {
        org.objectweb.asm.ClassReader classInfo = classReader;
        ...
        while (classInfo != null) {
            ...
            for (String name : interfaceNames) {
                if(name != null) {
                    final org.objectweb.asm.ClassReader interfaceInfo = getClassReader(name);
    ...
    private boolean isInterface(final org.objectweb.asm.ClassReader classReader) {
        return (classReader.getAccess() & Opcodes.ACC_INTERFACE) != 0;
    }

    private String getSuperClassName(final String className) {
        final org.objectweb.asm.ClassReader classReader = getClassReader(className);
        if (classReader == null) {
            return null;
        }
    ...
    private org.objectweb.asm.ClassReader getClassReader(final String className) {
        InputStream inputStream = urlClassLoader.getResourceAsStream(className + ".class");
        try {
            if (inputStream != null) {
                return new org.objectweb.asm.ClassReader(inputStream);
            }
